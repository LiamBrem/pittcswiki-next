...
const DATA: Tier[] = [
    {
        name: "S Tier",
        description: "Rough",
        color: "bg-red-50 dark:bg-red-500/10 text-red-900 dark:text-red-100 border-l-4 border-red-500 dark:border-red-500",
        courses: [
    ...
    },
    {
        ...
        color: "bg-orange-50 dark:bg-orange-500/10 text-orange-900 dark:text-orange-100 border-l-4 border-orange-500 dark:border-orange-500",
        courses: [
    ...
    },
    {
        ...
        color: "bg-yellow-50 dark:bg-yellow-500/10 text-yellow-900 dark:text-yellow-100 border-l-4 border-yellow-500 dark:border-yellow-500",
        courses: [
    ...
    },
    {
        ...
        color: "bg-blue-50 dark:bg-blue-500/10 text-blue-900 dark:text-blue-100 border-l-4 border-blue-500 dark:border-blue-500",
        courses: [
            ...
            { code: "CS 1666", name: "Principles of Computer Game Design and Implementation" },
    ...
    },
    {
        name: "D Tier",
        description: "Not Bad",
        color: "bg-green-50 dark:bg-green-500/10 text-green-900 dark:text-green-100 border-l-4 border-green-500 dark:border-green-500",
        courses: [
    ...
    return (
        <div className="w-full overflow-hidden rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm my-8 bg-white dark:bg-[#1a1a1a]">
            <div className="grid grid-cols-[1fr,1.5fr] bg-slate-50 dark:bg-black/20 border-b border-slate-200 dark:border-slate-800 text-sm font-semibold text-slate-500 dark:text-slate-400">
                ...
                <div className="px-6 py-3 border-l border-slate-200 dark:border-slate-800">Recent Professors</div>
            </div>
            ...
            <div className="divide-y divide-slate-200 dark:divide-slate-800">
                {DATA.map((tier) => (
                    <div key={tier.name} className="group">
                        {/* Tier Header */}
                        <div className={`px-6 py-2 text-xs font-bold uppercase tracking-wider border-b border-slate-200 dark:border-slate-800 ${tier.color}`}>
                        ...
                        <div className="divide-y divide-slate-100 dark:divide-slate-800/50 bg-white dark:bg-transparent">
                            {tier.courses.map((course) => {
                                ...
                                const courseData = CourseInfoData.courses.find(c => c.id === normalizedCode)
                                const instructors = courseData?.instructors || []

                                return (
                                    <div key={course.code} className="grid grid-cols-[1fr,1.5fr] hover:bg-slate-50 dark:hover:bg-white/5 transition-colors">
                                        <div className="px-6 py-3 text-sm text-slate-700 dark:text-slate-200 font-medium">
                                            <span className="font-mono text-slate-500 dark:text-slate-500 mr-2">{course.code}</span>
                                            {course.name}
                                        </div>
                                        <div className="px-6 py-3 text-sm text-slate-600 dark:text-slate-400 border-l border-slate-100 dark:border-slate-800/50 flex flex-wrap gap-x-2 gap-y-1">
                                            {instructors.length > 0 ? (
                                                instructors.map((prof, i) => (
                                                    <span key={prof}>
                                                        {prof}
                                                        {i < instructors.length - 1 && <span className="text-slate-300 dark:text-slate-600">,</span>}
                                                    </span>
                                                ))
                                            ) : (
                                                <span className="text-slate-400 italic text-xs">Check Course Explorer</span>
                                            )}
                                        </div>
                                    </div>
                                )
